...
export const promisePatch = (
  request: PatchRequest,
  rest: {
    patch: RestInterface['patch'],
  },
) => rest.patch(request.url, request.body, { headers: request.headers })
.then((response: AxiosResponse) => response.data)
.catch((err: Error) => {
  windowOrGlobal.console.error(`API ERROR: could not PATCH to ${request.url}: ${err.message}\n\n${err.stack}`)
  return Promise.reject(err)
});
